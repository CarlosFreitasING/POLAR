#& ".\.venv\Scripts\python.exe" ".\new\process_all_workers_tempvar.py"
import csv
import math
...
FEATURE_HEADER = [
    ...
    "HR_max",
    "HR_min",
    "HR_mean_ultimos_5min",
    "cambio_HR_vs_baseline",
    "tendencia_HR",
    "cambio_RMSSD",
]

BASE_FEATURES = FEATURE_HEADER[4:16]
TEMPORAL_FEATURES = FEATURE_HEADER[16:]
...
def add_temporal_features(feature_rows):
    """Add causal five-minute HR and consecutive-window change features."""
    base_rows = [
        row
    ...
    hr_mean_index = FEATURE_HEADER.index("HR_mean")
    rmssd_index = FEATURE_HEADER.index("RMSSD")
    baseline_hr = (
        sum(row[hr_mean_index] for row in base_rows) / len(base_rows)
        if base_rows
        else None
    )
    ...
    for row_index, row in enumerate(feature_rows):
        hr_value = row[hr_mean_index]
        rmssd_value = row[rmssd_index]
        previous_hr = [
            previous_row[hr_mean_index]
            for previous_row in feature_rows[max(0, row_index - 4):row_index + 1]
            if previous_row[hr_mean_index] != ""
        ]
        hr_last_5 = sum(previous_hr) / len(previous_hr) if previous_hr else ""

        if len(previous_hr) >= 2:
            x_values = list(range(len(previous_hr)))
            x_mean = sum(x_values) / len(x_values)
            y_mean = sum(previous_hr) / len(previous_hr)
            denominator = sum((x - x_mean) ** 2 for x in x_values)
            hr_trend = (
                sum((x - x_mean) * (y - y_mean) for x, y in zip(x_values, previous_hr))
                / denominator
                if denominator
                else ""
            )
        ...
        previous_rmssd = ""
        for previous_row in reversed(feature_rows[:row_index]):
            if previous_row[rmssd_index] != "":
                previous_rmssd = previous_row[rmssd_index]
                break

        temporal_rows.append(row + [
            hr_last_5,
            hr_value - baseline_hr if hr_value != "" and baseline_hr is not None else "",
            hr_trend,
            rmssd_value - previous_rmssd
            if rmssd_value != "" and previous_rmssd != ""
            else "",
        ])
...
def normalize_features(feature_rows):
    ...
    for feature in NORMALIZATION_FEATURES:
        values = [
            ...
            if row[FEATURE_HEADER.index(feature)] != ""
            and math.isfinite(row[FEATURE_HEADER.index(feature)])
        ]
