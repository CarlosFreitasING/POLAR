...
# Find the sequential pc-#.csv files in the same folder as this script.
data_dir = Path(__file__).resolve().parent
files = [
    path for path in data_dir.glob('* pc-*.csv')
    if re.search(r'pc-\d+\.csv$', path.name, re.IGNORECASE)
]
files.sort(
    key=lambda path: int(
        re.search(r'pc-(\d+)\.csv$', path.name, re.IGNORECASE).group(1)
    )
)

if not files:
    raise FileNotFoundError(f'No se encontraron archivos pc-#.csv en {data_dir}')

print("Starting processing files")
combined_file = data_dir / 'combined_features.csv'
all_rows = []
...
# Calculate global statistics for z-score normalization using W01 as reference
features_indices = {
    'ECG_energy': 5,
    'ECG_samp_ent': 6,
    'ECG_missing_peaks': 7,
...
reference_file = data_dir.parent.parent / 'W01' / 'DARA_PROC' / 'combined_features.csv'
if not reference_file.exists():
    raise FileNotFoundError(f'No se encontro el archivo de referencia de W01: {reference_file}')

reference_values = {name: [] for name in features_indices}
with open(reference_file, 'r', newline='', encoding='utf-8-sig') as infile:
    reader = csv.DictReader(infile)
    for source_row in reader:
        for name in features_indices:
            value = float(source_row[name])
            if value != 0 or name in ['ECG_energy', 'ECG_samp_ent', 'ECG_missing_peaks']:
                reference_values[name].append(value)
...
try:
    with open(combined_file, 'r', newline='', encoding='utf-8') as infile:
        reader = csv.reader(infile)
        header = next(reader)
        ...
        for row in reader:
            ...
            if len(current_chunk) >= chunk_size:
                # Write chunk
                chunk_file = data_dir / f'features_part_{chunk_num}.csv'
                with open(chunk_file, 'w', newline='', encoding='utf-8') as outfile:
                    writer = csv.writer(outfile)
                    writer.writerow(header)
